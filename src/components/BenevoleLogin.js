...
export class BenevoleLogin extends React.Component {
  constructor() {
    super();
  ...
  send = async () => {
    ...
    try {
      const { data } = await API.benevoleLogin(email, password);
      localStorage.setItem("benevoleToken", data.benevoleToken);
